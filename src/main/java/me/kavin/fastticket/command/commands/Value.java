...
public class Value extends Command {
	...
	public void onCommand(String message, MessageReceivedEvent event) throws Throwable {
...

		{
			...
			if (chosen != null)
				...
				case CATEGORY: {
					String q = null;
					for (int i = (getPrefix() + " " + ValueType.CATEGORY.name()).length() + 1; i < message
							.length(); i++) {
						if (q == null)
							q = "";
						q += message.charAt(i);
					...
					if (q == null) {
						meb.setDescription("Please specify a category by name to set to.");
					} else if (q.equalsIgnoreCase("none")) {
						SettingsHelper.getInstance().setTicketCategory(event.getGuild().getIdLong(), null);
						meb.setDescription("Reset the category config.");
					} else {
						List<Category> categories = event.getGuild().getCategoriesByName(q, true);
						if (!categories.isEmpty()) {
							Category category = categories.get(0);
							SettingsHelper.getInstance().setTicketCategory(event.getGuild().getIdLong(), category);
							meb.setDescription("Set the category to " + category.getId() + ".");
						} else {
							meb.setDescription("Invalid category specified.");
						}
					}
				}
					break;
	...
	private static enum ValueType {
		COLOR, LOGS, TICKET_OPEN_MSG, ROLE, OPEN_TAG, PING_OPENER, CLOSE_INACTIVE, FORCE_REASON, CATEGORY
	}
}